...
Мы как всегда будем хитрить и писать некоторыми оптимизациями.
...
Т.е. число `852493284923849834982` в векторе будет выглядить как-то так:
```rust
...
impl BigUInt {
    fn from_vec<T: Into<Vec<u8>>>(digit: T) -> BigUInt {
        // ничего сложного, просто наш T должен реализовывать трейт Into преобразующих digit в вектор
        BigUInt {
            digit: digit.into(),
        }
    }
...
- пройтись по вслем элементам и сложить с учётом переноса
...
Я знаю что это не лучший варинат, но он прост в реализации и понятен.
...
Мы же ограничились выбором минимальным времени из 10 тестов для каждого из примеров. 
...
Если мне будет скучно, то я добавлю реализацию ещё на паре языков.
